...
function InboxCard({ kind, kindColor, title, ignore = false }: { kind: string; kindColor: string; title: string; ignore?: boolean }) {
  return (
    <div style={{ border: "1px solid var(--line2)", borderLeft: `2px solid ${kindColor}`, background: "var(--tint)", padding: "8px 10px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4 }}>
        <span style={{ fontSize: 7.5, letterSpacing: ".12em", color: kindColor }}>{kind}</span>
        <span style={{ fontSize: 10, color: "var(--strong)", flex: 1 }}>{title}</span>
      </div>
      <div style={{ display: "flex", gap: 6 }}>
        <span style={{ fontSize: 8, color: "var(--on-accent)", background: kindColor, padding: "3px 9px", fontWeight: 600 }}>ACCEPT</span>
    ...
    <PanelShell {...props}>
      <div data-screen-label="PSYKE Console + Auto-Link Inbox" style={panelBox}>
        ...
        <div style={{ padding: 13, borderBottom: "1px solid var(--line)" }}>
          ...
          <div style={{ border: "1px solid var(--line-cy)", background: "var(--raised)", boxShadow: "0 0 22px rgba(76,194,255,.12)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "10px 12px", borderBottom: "1px solid var(--line2)" }}>
              ...
              <span style={{ fontSize: 12, color: "var(--strong)" }}>make vesper colder<span style={{ display: "inline-block", width: 2, height: 14, background: "var(--accent)", verticalAlign: -2, marginLeft: 1, animation: "lf-blink 1.1s step-end infinite" }} /></span>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "8px 12px", background: "rgba(76,194,255,.08)", borderLeft: "2px solid var(--accent)" }}>
                <span style={{ color: "var(--amber)" }}>⚡</span><span style={{ fontSize: 11, color: "var(--strong)", flex: 1 }}>Rewrite — colder register</span>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "8px 12px" }}>
                <span style={{ color: "var(--cyan)" }}>◆</span><span style={{ fontSize: 11, color: "var(--txt)", flex: 1 }}>Open <span style={{ color: "var(--strong)" }}>VESPER</span></span><span style={{ fontSize: 7.5, color: "var(--txt3)" }}>entity · scene-boosted</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "8px 12px" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 13 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 7.5, color: "var(--on-accent)", background: "var(--amber)", padding: "1px 6px", borderRadius: 6, fontWeight: 600 }}>4 PENDING</span>
