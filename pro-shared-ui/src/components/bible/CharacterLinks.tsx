...
export function CharacterLinks(props: PanelProps) {
  ...
  const [busy, setBusy] = useState<number | "auto" | null>(null);
    ...
    <PanelShell {...props}>
      <div data-screen-label="Character Links" style={panelBox}>
        ...
        <div style={{ height: 40, flex: "none", display: "flex", alignItems: "center", gap: 13, padding: "0 16px", borderBottom: "1px solid var(--line)" }}>
          <span style={{ fontFamily: "'Chakra Petch'", fontWeight: 600, fontSize: 13, letterSpacing: ".12em", color: "#fff" }}>CHARACTER LINKS</span>
          ...
          : (
            <div style={{ flex: 1, overflowY: "auto", padding: "10px 14px", display: "flex", flexDirection: "column", gap: 7 }}>
              ...
              {cast.map((c) => {
                ...
                return (
                  <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 11, padding: "8px 11px", border: `1px solid ${isLinked ? "var(--line2)" : "rgba(245,177,51,.35)"}`, background: isLinked ? "rgba(11,14,21,.55)" : "rgba(245,177,51,.05)", opacity: busy === c.id ? 0.5 : 1 }}>
                    <span style={{ width: 9, height: 9, flex: "none", borderRadius: "50%", background: c.color || "var(--accent)" }} />
                    <span style={{ fontFamily: "'Chakra Petch'", fontSize: 12, color: "#fff", letterSpacing: ".02em", minWidth: 130 }}>{c.name}</span>
                    ...
                    <select
                      ...
                      style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, color: "var(--txt)", background: "#11151e", border: "1px solid var(--line2)", padding: "4px 7px", minWidth: 200 }}
                    >
