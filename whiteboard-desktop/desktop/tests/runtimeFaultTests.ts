...
} from '../renderer/src/components/runtimeFaults';
...
check(
  'fault identity is stable across sources',
  textFault.key === createRuntimeFault('promise', 'broken click', 1200).key,
);
...
check('hostile proxy reasons never break reporting', createRuntimeFault('promise', hostile).message.length > 0);
...
check(
  'an ordinary error mentioning abort is still reported',
  !isExpectedCancellation(new Error('request aborted unexpectedly')),
);

check('first occurrence is reportable', shouldReportRuntimeFault(null, fault));
check(
  'same occurrence is deduplicated inside the window',
  !shouldReportRuntimeFault({ key: fault.key, occurredAt: 1000 }, { ...fault, occurredAt: 5999 }),
);
check(
  'same occurrence returns at the window boundary',
  shouldReportRuntimeFault({ key: fault.key, occurredAt: 1000 }, { ...fault, occurredAt: 6000 }),
);
check(
  'different errors are never collapsed',
  shouldReportRuntimeFault({ key: fault.key, occurredAt: 1000 }, textFault),
);
