...
- **Save conflict** (`store.saveError` with `saveState === 'conflict'`): a persistent inline alert is shown above the still-visible outliner. Automatic persistence pauses and retains subsequent local edits instead of replacing them. The alert can export the retained outline as JSON or open a confirmation before adopting the latest saved outline. A versioned copy also remains in main's crash-safe recovery journal across renderer and process restarts and appears in the app-lifetime recovery banner until exact resolution. The exported outline recovery can be restored through Import LogosForge; it replaces only the outline and never blanks or appends to the manuscript.
